 ...
 * extension.
 */
class CRM_Genderselfidentify_ExtensionUtil {
  const SHORT_NAME = "genderselfidentify";
  const LONG_NAME = "org.woolman.genderselfidentify";
  const CLASS_PREFIX = "CRM_Genderselfidentify";
 ...
 */
function _genderselfidentify_civix_civicrm_postInstall() {
  _genderselfidentify_civix_civicrm_config();
  if ($upgrader = _genderselfidentify_civix_upgrader()) {
 ...
 */
function _genderselfidentify_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _genderselfidentify_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 *    filled for you)
 */
function _genderselfidentify_civix_insert_navigation_menu(&$menu, $path, $item) {
 ...
 */
function _genderselfidentify_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _genderselfidentify_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _genderselfidentify_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _genderselfidentify_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _genderselfidentify_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _genderselfidentify_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
...
function _genderselfidentify_civix_civicrm_entityTypes(&$entityTypes) {
